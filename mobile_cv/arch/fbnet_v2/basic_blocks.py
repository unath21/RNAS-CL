...
class ConvBNRelu(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        teacher_length = 45
        self.kd_GS_thetas = nn.Parameter(torch.ones(teacher_length)*(1/teacher_length))        
    ...
    def forward(self, x, temperature, teacher_dict):
        if x.numel() > 0 or self.empty_input is None:
            if self.conv:
                x = self.conv(x)
                soft_mask_variables = nn.functional.gumbel_softmax(self.kd_GS_thetas, temperature)
                for i in range(self.teacher_length):
                    student_weight, teacher_weight = interpolate_to_same_shape(self.conv.weight, teacher_dict[i])
                    ...
                    kl_loss += kl * soft_mask_variables[i]
            if self.bn:
                x = self.bn(x)
            if self.relu:
                x = self.relu(x)
