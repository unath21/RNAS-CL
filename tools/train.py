...
from engine.trainer import do_train, do_train_with_center, do_train_dnl_with_center
...
def train(cfg):
    ...
    if cfg.MODEL.CONTINUE:
        model.load_trained(cfg.MODEL.CONTINUE_PATH)


    # if cfg.MODEL.WEIGHTS_OPEN:
    ...
    if cfg.MODEL.DNL:
        ...
        optimizer, optimizer_center, optimizer_dnl = make_dnl_optimizer_with_center(cfg, model, center_criterion)
        ...
        if cfg.DNL.SEPARATE_SEARCH_SCHEDULER:
            scheduler = WarmupMultiStepLR_reinit(optimizer, cfg.SOLVER.STEPS, cfg.SOLVER.GAMMA, cfg.SOLVER.WARMUP_FACTOR,
                                                 cfg.SOLVER.WARMUP_ITERS + cfg.DNL.MAX_SEARCH_EPOCH, cfg.SOLVER.WARMUP_METHOD, cfg.DNL.MAX_SEARCH_EPOCH,
        ...
        if cfg.DNL.LR_SCHEDULER == 'WarmupMultiStepLR':
            scheduler_dnl = WarmupMultiStepLR(optimizer_dnl, cfg.DNL.STEPS, cfg.DNL.GAMMA, cfg.DNL.WARMUP_FACTOR,
                                              cfg.DNL.WARMUP_ITERS, cfg.DNL.WARMUP_METHOD)
        elif cfg.DNL.LR_SCHEDULER == 'CosineAnnealingLr':
            scheduler_dnl = CosineAnnealingLR(optimizer_dnl, T_max=cfg.DNL.MAX_SEARCH_EPOCH + 1, eta_min=cfg.DNL.STARTPOS_LR)

        do_train_dnl_with_center(
            cfg,
            model,
            center_criterion,
            train_loader,
    ...
    elif cfg.MODEL.IF_WITH_CENTER == 'no':
        ...
        if cfg.MODEL.PRETRAIN_CHOICE == 'self':
            ...
            # model.load_state_dict(torch.load(cfg.MODEL.PRETRAIN_PATH))
            model.load_trained(cfg.MODEL.PRETRAIN_PATH)



            # optimizer_dict = torch.load(path_to_optimizer)
            # for k, v in optimizer_dict.state_dict().items():
            #     optimizer.state_dict()[k].copy_(optimizer_dict.state_dict()[k])

            optimizer = torch.load(path_to_optimizer)
            center_criterion = load_param(center_criterion, path_to_center_param)
...
def main():
    ...
    if cfg.TEST.IF_WDB:
        wandb.init(
            name=cfg.TEST.WDB_NAME,
            project=cfg.TEST.WDB_PROJECT,
            config=cfg,
            dir='/data/yyang409/yancheng/wandb',
        )
    train(cfg)
