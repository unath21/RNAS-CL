...
def main():
    ...
    grad_cam = GradCam(model=model, feature_module=model.base.conv9, target_layer_names=[cfg.GRADCAM.TARGET_LAYER_NAMES], use_cuda=use_cuda)
    ...
    show_cam_on_image(img, mask, cfg.GRADCAM.OUT_PATH)
    ...
    path = cfg.GRADCAM.OUT_PATH
    # cv2.imwrite(os.path.join(path, 'gb.jpg'), gb)
    # cv2.imwrite(os.path.join(path, 'cam_gb.jpg'), cam_gb)
...
class GradCam:
    ...
    def __call__(self, input, index=None):
        ...
        if index == None:
            ...
            print("We visualized the gradients for the index with highest score" + str(index))
            print("The index with highest score is " + str(index) + ", and the score is " + str(np.max(output.cpu().data.numpy())))
        else:
            print("We visualized the gradients for index " + str(index))
            print("The score for index  " + str(index) + " is " + str(output[0][index].cpu().data.numpy()) +
                  " and highest score is " + str(np.max(output.cpu().data.numpy())) + " for index " + str(np.argmax(output.cpu().data.numpy())))
