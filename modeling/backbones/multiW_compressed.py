...
class MobileNetV2_deep(nn.Module):
    def __init__(
        ...
        self.conv6 = self._make_layer_compressed(multiW_Bottleneck, 6, int(96 * width_mult), structure[4], 1)
    ...
    def _make_layer_compressed(self, block, t, c, n, s):
        # t: expansion factor
        # c: output channels
        # n: number of blocks
        # s: stride for first layer
        layers = []
        layers.append(block(self.in_channels, c, t, s, first_unit=True))
        self.in_channels = c
        for i in range(1, n):
            if i == n-1:
                last_unit = True
            ...
            layers.append(block(self.in_channels, c, t, last_unit=last_unit))
        return nn.Sequential(*layers)
...
def multiW_compressed_mobilenetv2_53(num_classes, last_stride=2, width_mult=1.0, **kwargs):
