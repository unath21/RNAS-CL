...
class HAMobile(nn.Module):
    ...
    def __init__(
        ...
        if self.learn_region:
            ...
            self.local_conv1 = self._make_layer_local(Bottleneck, 6, 32, nchannels[0], 2, 2)
            self.local_conv2 = self._make_layer_local(Bottleneck, 6, nchannels[0], nchannels[1], 2, 2)
            self.local_conv3 = self._make_layer_local(Bottleneck, 6, nchannels[1], nchannels[2], 2, 2)
...
def ha_mobilenet_global_local(cfg, num_classes, use_gpu=True, **kwargs):
    model = HAMobile(
        cfg,
        num_classes,
        local_resolution=[24, 28],
...
def ha_mobilenet_global_only(cfg, num_classes, use_gpu=True, **kwargs):
    model = HAMobile(
        cfg,
        num_classes,
        local_resolution=[24, 28],
