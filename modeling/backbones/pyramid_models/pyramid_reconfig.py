...
import torch
import warnings
from .mobilenet_base import SqueezeAndExcitation, get_active_fn
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        c_afterConcat = int(32 * width_mult + 96 * width_mult)
        se_ratio = 0.5
        active_fn = get_active_fn('nn.Swish')
        self.se = SqueezeAndExcitation(c_afterConcat, int(c_afterConcat * se_ratio), active_fn=active_fn)
        layer_after_pyramid = 1
        self.in_channels = c_afterConcat
        self.pyramid_branch = self._make_layer(Bottleneck, 6, int(320 * width_mult), layer_after_pyramid, 2)
        self.in_channels = int(96 * width_mult)
        ...
        # self.upSample_stage4 = nn. Upsample(scale_factor=2, mode='nearest')
    ...
    def featuremaps(self, x):
        ...
        x = self.conv2(x)
        x = self.conv3(x)
        out_stage2 = self.conv4(x)

        x = self.conv5(out_stage2)
        out_stage3 = self.conv6(x)

        up_stage3 = self.upSample_stage3(out_stage3)

        x = self.conv7(out_stage3)
        out_stage4 = self.conv8(x)

        lower_map = torch.cat((out_stage2, up_stage3), dim=1)
        lower_map = self.se(lower_map)
        lower_map = self.pyramid_branch(lower_map)
        combined_stage4 = out_stage4 + lower_map

        x = self.conv9(combined_stage4)
        return x
...
def mobilenetv2_pyramidRC(num_classes, last_stride=1, pre_train=False, **kwargs):
