...
class MobileNetV2_large(nn.Module):
    """MobileNetV2.
    ...
    def __init__(
        ...
        super(MobileNetV2_large, self).__init__()
        #self.loss = loss
        ...
        self.conv2 = self._make_layer(Bottleneck, 1, int(16 * width_mult), 1+1, 1)
        self.conv3 = self._make_layer(Bottleneck, 6, int(24 * width_mult), 2+2, 2)
        self.conv4 = self._make_layer(Bottleneck, 6, int(32 * width_mult), 3+3, 2)
        self.conv5 = self._make_layer(Bottleneck, 6, int(64 * width_mult), 4+4, 2)
        self.conv6 = self._make_layer(Bottleneck, 6, int(96 * width_mult), 3+6, 1)
        self.conv7 = self._make_layer(Bottleneck, 6, int(160 * width_mult), 3+1, 2)
        self.conv8 = self._make_layer(Bottleneck, 6, int(320 * width_mult), 1+1, 1)
...
def mobilenetv2_x1_0_large(num_classes, pretrain_choice='imagenet', **kwargs):
    ...
    if pretrain_choice == 'none':
        model._init_params()
    return model



def mobilenetv2_x1_4_large(num_classes, pretrain_choice='imagenet', **kwargs):
    model = MobileNetV2_large(
        num_classes,
        # loss=loss,
        width_mult=1.4,
...
def mobilenetv2_x2_0_large(num_classes, pretrain_choice='imagenet', **kwargs):
    model = MobileNetV2_large(
        num_classes,
        # loss=loss,
        width_mult=2.0,
