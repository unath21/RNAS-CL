...
from .SuperConv.multiW_SuperConv import multiW_SuperConv
...
class multiW_Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1):
        ...
        self.conv1 = multiW_ConvBlock(super_in_channels=in_channels,
                                      ...
                                      stride=1,
                                      groups=1,
                                      num_window=mid_channels,
                                      window_length=in_channels,
                                      real_inchannels=in_channels,
                                      real_outchannels=mid_channels
                                      )
        ...
        self.conv3 = nn.Sequential(
            multiW_SuperConv(
                ...
                stride=1,
                groups=1,
                num_window=out_channels,
                window_length=mid_channels,
                real_inchannels=mid_channels,
                real_outchannels=out_channels
            ),
            nn.BatchNorm2d(out_channels),
        )
...
class MobileNetV2_deep(nn.Module):
    ...
    def __init__(
        ...
        self.conv6 = self._make_layer(multiW_Bottleneck, 6, int(96 * width_mult), structure[4], 1)
...
def multiW_mobilenetv2_53(num_classes, last_stride=2, width_mult=1.0, **kwargs):
    ...
    pth_load_pretrained_weights(model,
...
def pth_load_pretrained_weights(model, weight_path):
    ...
    """
    checkpoint_baseline = pth_load_checkpoint(weight_path)
