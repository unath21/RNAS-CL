...
class MobileNetV3(nn.Module):
    ...
    def as_sequential(self):
        ...
        # layers.extend(
        #     [self.global_pool, self.conv_head, self.act2, nn.Flatten(), nn.Dropout(self.drop_rate), self.classifier]
        # )
        layers.extend(
            [self.global_pool, self.conv_head]
        )
...
def _gen_mobilenet_v3(variant, channel_multiplier=1.0, last_stride=2,pretrained=False, **kwargs):
    ...
    if 'small' in variant:
        ...
        if 'minimal' in variant:
            ...
            act_layer = 'hard_swish'
            stage5 = 'ir_r3_k5_s' + str(last_stride) + '_e6_c160_se0.25'
            arch_def = [
                # stage 0, 112x112 in
...
def mobilenetv3_large_100(pretrained=False, last_stride=2, **kwargs):
    """ MobileNet V3 Large 1.0 """
    # NOTE for train set drop_rate=0.2
    model = _gen_mobilenet_v3('mobilenetv3_large_100', 1.0, last_stride=last_stride, pretrained=pretrained, **kwargs)
    return model
