...
import warnings
import pickle
from .mobilenet_base import MyNonlocal
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        self.non_local_1 = MyNonlocal(int(24 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv4 = self._make_layer(Bottleneck, 6, int(32 * width_mult), structure[2], 2)
        self.non_local_2 = MyNonlocal(int(32 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv5 = self._make_layer(Bottleneck, 6, int(64 * width_mult), structure[3], 2)
        self.non_local_3 = MyNonlocal(int(64 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        ...
        self.non_local_4 = MyNonlocal(int(320 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
...
def mobilenetv2_53_stage_lightNL_My(num_classes, last_stride=1, pre_train=False, nl_c=0.5, **kwargs):
...
def mobilenetv2_300_stage_lightNL_My(num_classes, last_stride=1, pre_train=False, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        nl_c=0.5,
...
def mobilenetv2_200_stage_lightNL_My(num_classes, last_stride=1, pre_train=False, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        nl_c=0.5,
