...
import warnings
import pickle
from .mobilenet_base import Nonlocal, ChannelNonlocal
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        if spatial_NL_opt:
            self.non_local_1 = Nonlocal(int(24 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        if channel_NL_opt:
            self.ch_non_local_1 = ChannelNonlocal(int(24 * width_mult), 1, 2, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv4 = self._make_layer(Bottleneck, 6, int(32 * width_mult), structure[2], 2)
        if spatial_NL_opt:
            self.non_local_2 = Nonlocal(int(32 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        if channel_NL_opt:
            self.ch_non_local_2 = ChannelNonlocal(int(32 * width_mult), 1, 2, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv5 = self._make_layer(Bottleneck, 6, int(64 * width_mult), structure[3], 2)
        if spatial_NL_opt:
            self.non_local_3 = Nonlocal(int(64 * width_mult), nl_c, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        if channel_NL_opt:
            self.ch_non_local_3 = ChannelNonlocal(int(64 * width_mult), 1, 2, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
...
def mobilenetv2_53_stage_double_lightNL(num_classes, last_stride=1,  sp_NL_opt=True, ch_NL_opt=False, pre_train=False, **kwargs):
...
def mobilenetv2_300_stage_lightNL(num_classes, last_stride=1, pre_train=False, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        nl_c=0.5,
