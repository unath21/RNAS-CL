...
import warnings
import pickle
from .mobilenet_base import Nonlocal_Linear
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        h = h / 2
        w = w / 2
        self.non_local_1 = Nonlocal_Linear(int(24 * width_mult), nl_c, 1, int(h*w), norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv4 = self._make_layer(Bottleneck, 6, int(32 * width_mult), structure[2], 2)
        h = h / 2
        w = w / 2
        self.non_local_2 = Nonlocal_Linear(int(32 * width_mult), nl_c, 1, int(h*w), norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)

        self.conv5 = self._make_layer(Bottleneck, 6, int(64 * width_mult), structure[3], 2)
        h = h / 2
        w = w / 2
        self.non_local_3 = Nonlocal_Linear(int(64 * width_mult), nl_c, 1, int(h*w), norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        ...
        self.non_local_4 = Nonlocal_Linear(int(320 * width_mult), nl_c, 1, int(h*w), norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
...
def mobilenetv2_53_stage_lightNL_linear(num_classes, last_stride=1, pre_train=False, nl_c=0.5, **kwargs):
