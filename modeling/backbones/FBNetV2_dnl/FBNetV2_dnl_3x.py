...
class FBNetV2_3x_dnl(nn.Module):
    def __init__(
            self,
            nl_c=1.0,
            last_stride=1,
        ...
        super(FBNetV2_3x_dnl, self).__init__()
        batch_norm_kwargs = {
            'momentum': batch_norm_momentum,
            'eps': batch_norm_epsilon
        }

        ori_model = fbnet("da_fbnetv2_3x", pretrained=False)
...
def FBNetV2_3x_dnl_searched(last_stride=2, pre_train=False, **kwargs):
    model = FBNetV2_3x_dnl(last_stride=last_stride)

    searched_start_position = [2.497, 10.354, 50.129, 39.979]
    searched_length = [18.239, 20.147, 57.233, 156.327]
