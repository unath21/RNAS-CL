...
class multiW_SuperConv(nn.Conv2d):
    def __init__(self, super_in_channels, super_out_channels, kernel_size, stride=1,
        ...
        self.theta_2_0 = torch.nn.Parameter(torch.Tensor([0.0, (window_length - 1) / (super_length - 1)]), requires_grad=False)
    ...
    def forward(self, input):
        ...
        for i in range(0, self.num_window):
            ...
            theta_2_0 = self.theta_2_0
            theta_2 = torch.cat((theta_2_0, cal_position_chunk[i]), dim=0)
            theta = torch.cat((theta_1, theta_2), dim=0)
            theta = theta.view(1, 2, 3)
            flow_field = torch.nn.functional.affine_grid(theta, size)
            sampled_weight = torch.nn.functional.grid_sample(weight.to(torch.float32),
                                                             flow_field.to(torch.float32))
            interpolated_weight = sampled_weight.to(torch.float32)
            if i == 0:
                interpolated_weight_all = interpolated_weight
            else:
                interpolated_weight_all = torch.cat((interpolated_weight_all, interpolated_weight), dim=2)
