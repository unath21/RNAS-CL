...
class Slimmed_Bottleneck(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, compress_ratio=0.9):

        super(Slimmed_Bottleneck, self).__init__()

        mid_channels = int(in_channels * expansion_factor)

        self.use_residual = stride == 1 and in_channels == out_channels

        self.conv1 = ConvBlock(int(in_channels * compress_ratio), int(mid_channels * compress_ratio), 1)

        self.dwconv2 = ConvBlock(int(mid_channels * compress_ratio), int(mid_channels * compress_ratio), 3, stride, 1, g=int(mid_channels * compress_ratio))

        self.conv3 = nn.Sequential(
            nn.Conv2d(int(mid_channels * compress_ratio), int(out_channels * compress_ratio), 1, bias=False),
            nn.BatchNorm2d(int(out_channels * compress_ratio)),
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, first_opt=False):
        ...
        if first_opt:
            out_channels = int(out_channels * 0.9)
        self.conv3 = nn.Sequential(
...
class Bottleneck_last_slimmed(nn.Module):

    def __init__(self, in_channels, out_channels, expansion_factor, stride=1, original_out=96):

        super(Bottleneck_last_slimmed, self).__init__()

        mid_channels = int(in_channels * expansion_factor * 0.9)

        self.use_residual = stride == 1 and in_channels == out_channels

        self.conv1 = ConvBlock(int(in_channels * 0.9), mid_channels, 1)
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(mid_channels, original_out, 1, bias=False),
            nn.BatchNorm2d(original_out),
        )
...
#################### Deeper Version of MobileNetV2 ######################
    ...
    MobileNetV2-299                 [6, 31, 57,  4]                         [1,  6, 31, 29, 28,  3,  1]
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        self.conv5 = self._make_first_slimmed(Bottleneck, 6, 64, structure[3], 2, 0.9)
        self.conv6 = self._make_last_slimmed(Bottleneck, 6, 96, structure[4], 1, 96)
    ...
    def _make_first_slimmed(self, block, t, c, n, s, compress_ratio=0.9):
        # t: expansion factor
        # c: output channels
        # n: number of blocks
        # s: stride for first layer
        layers = []
        layers.append(block(self.in_channels, c, t*compress_ratio, s, first_opt=True))
        self.in_channels = c
        for i in range(1, n):
            layers.append(Slimmed_Bottleneck(self.in_channels, c, t, compress_ratio=0.9))
        return nn.Sequential(*layers)

    def _make_last_slimmed(self, block, t, c, n, s, original_out):
        # t: expansion factor
        # c: output channels
        # n: number of blocks
        # s: stride for first layer
        layers = []
        layers.append(Slimmed_Bottleneck(self.in_channels, c, t, s, compress_ratio=0.9))
        self.in_channels = c
        for i in range(1, n):
            if i == n-1:
                layers.append(Bottleneck_last_slimmed(self.in_channels, c, t, original_out=original_out))
                self.in_channels = original_out
            else:
                layers.append(Slimmed_Bottleneck(self.in_channels, c, t, compress_ratio=0.9))
        return nn.Sequential(*layers)
...
def mobilenetv2_300_w1S309_baseline(num_classes, last_stride=1, width_mult=1, **kwargs):
