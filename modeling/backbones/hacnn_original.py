...
class HACNN_FLEX(nn.Module):
    ...
    def __init__(
        ...
        super(HACNN_FLEX, self).__init__()
    ...
    def stn(self, x, theta):
        ...
        """
        grid = F.affine_grid(theta, x.size())
        if self.use_gpu: grid = grid.cuda()
        x = F.grid_sample(x, grid)
        return x
...
def hacnn_256_40_56(cfg, num_classes):
    model = HACNN_FLEX(
        num_classes,
        [40, 56],
...
def hacnn_256_24_28(cfg, num_classes):
    model = HACNN_FLEX(
        num_classes,
        [24, 28],
        neck_feat=cfg.TEST.NECK_FEAT,
...
def hacnn_256_80_56(cfg, num_classes):
    model = HACNN_FLEX(
        num_classes,
        [40, 56],
        neck_feat=cfg.TEST.NECK_FEAT,
...
def hacnn_oroginal(cfg, num_classes):
    model = HACNN_FLEX(
        num_classes,
        [24, 28],
        neck_feat=cfg.TEST.NECK_FEAT,
