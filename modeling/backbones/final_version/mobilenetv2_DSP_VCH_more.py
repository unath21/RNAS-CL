...
def mobilenetv2_53_DSP_VCH(num_classes, pre_train=False, sp_NL_opt=True, ch_NL_opt=True, VCH_nls=2, **kwargs):
...
def mobilenetv2_200_DSP_VCH(num_classes, pre_train=False, sp_NL_opt=True, ch_NL_opt=True, VCH_nls=2, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        initial_nl_c=0.5,
    ...
    model._init_params()
    if not pre_train:
        warnings.warn("Training mobilenetv2_200 from scratch.")
    if pre_train:
        warnings.warn("Training mobilenetv2_200 from scratch.")
    return model
