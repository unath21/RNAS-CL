...
class DNonlocal(nn.Module):
    ...
    def __init__(self, n_feature, nl_c, nl_s, norm_method='batch_norm', batch_norm_kwargs=None):
        ...
        self.ch_startpos_int = torch.nn.Parameter(startpos_int.to(torch.int8), requires_grad=False)
    ...
    def forward(self, l):
        ...
        theta_1 = l_padded[:, startpos_int:startpos_int + self.window_length, :, :]

        theta_2 = l_padded[:, startpos_int + 1:startpos_int + self.window_length + 1, :, :]

        phi_1 = lr_padded[:, startpos_int:startpos_int + self.window_length, :, :]

        phi_2 = lr_padded[:, startpos_int + 1:startpos_int + self.window_length + 1, :, :]

        theta = theta_1 * (1 - self.ch_startpos_dec) + theta_2 * self.ch_startpos_dec
        phi = phi_1 * (1 - self.ch_startpos_dec) + phi_2 * self.ch_startpos_dec

        if (H * W) * reduced_HW * channel_in * (1 + self.nl_c) \
                < \
                (H * W) * channel_in**2 * self.nl_c + reduced_HW * channel_in**2 * self.nl_c:
