...
import warnings
import pickle
from .DNLblock_v2 import DNonlocal
...
Stage4  conv7:  14  x 14  x 96      14  x 14  x 160     3                   9
    ...
    def __init__(
        ...
        # self.non_local_2 = Nonlocal(int(32 * width_mult), 1, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        ...
        # self.non_local_3 = Nonlocal(int(64 * width_mult), 1, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
        ...
        # self.non_local_4 = Nonlocal(int(320 * width_mult), 1, 1, norm_method=nl_norm_method, batch_norm_kwargs=batch_norm_kwargs)
    ...
    # def _get_NL1_ch_startpos_int(self):
    #     return self.non_local_1.ch_startpos_int

    # def _set_NL1_ch_startpos_dec(self, ch_startpos_dec):
    #
    #     # startpos_dec = torch.from_numpy(np.array([ch_startpos_dec])).cuda()
    #     # self.non_local_1.ch_startpos_dec = torch.nn.Parameter(startpos_dec.to(torch.float32), requires_grad=True)
    #
    #     startpos_dec = torch.from_numpy(np.array(ch_startpos_dec)).cuda()
    #     self.non_local_1.ch_startpos_dec.data = startpos_dec
...
def mobilenetv2_53_stage_dNL_bothv2(num_classes, last_stride=1, pre_train=False, **kwargs):
...
def mobilenetv2_200_stage_dNL_bothv2(num_classes, last_stride=1, pre_train=False, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        initial_nl_c=0.5,
...
def mobilenetv2_300_stage_dNL_bothv2(num_classes, last_stride=1, pre_train=False, **kwargs):
    model = MobileNetV2_deep(
        num_classes,
        width_mult=1,
        initial_nl_c=0.5,
