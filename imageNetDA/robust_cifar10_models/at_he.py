...
class AT_HE(torch.nn.Module):
    ...
    def forward(self, x):
        labels = self.model(x.cuda(self.device))
        return labels
    ...
    def load(self):
        ...
        for k,v in self.model.state_dict().items():
            self.model.state_dict()[k].copy_(checkpoint['state_dict']['module.'+k]) 
...
class WideResNet(nn.Module):
    ...
    def forward(self, x):
        if self.i_normalize:
            self.mu = torch.tensor((0.4914, 0.4822, 0.4465)).view(3,1,1).cuda(0)
            self.std = torch.tensor((0.2471, 0.2435, 0.2616)).view(3,1,1).cuda(0)
