...
class CustumCifarDataset(torchvision.datasets.CIFAR10):
...
def create_dataset(config: yacs.config.CfgNode,
    ...
    if config.dataset.name in [
        ...
        train_transform = create_transform(config, is_train=True)
        val_transform = create_transform(config, is_train=False)
        train_dataset = torchvision.datasets.ImageFolder(dataset_dir / 'train', transform=train_transform)
        # train_dataset = torchvision.datasets.ImageFolder('/scratch/work/public/imagenet/train', transform=train_transform)
        val_dataset = torchvision.datasets.ImageFolder(dataset_dir / 'val',transform=val_transform)
        return train_dataset, val_dataset
    else:
        raise ValueError()
