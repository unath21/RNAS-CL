...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, fake_relu=False, no_relu=False):
        ...
        out = self.relu(out)
        out1 = (out**2).sum(1)
        out1 = F.interpolate(out1[None,:], size =(28,28), mode='bilinear')
        ...
        out = self.relu(out)
        out2 = (out**2).sum(1)
        out2 = F.interpolate(out2[None,:], size =(28,28), mode='bilinear')
        print(all_out.shape, out1.shape)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, fake_relu=False, no_relu=False):
        ...
        out1 = (out**2).sum(1)
        out1 = F.interpolate(out1[None,:], size =(28,28), mode='bilinear')
        #out1 = F.interpolate(out[None,:], size =(128,14,14), mode='trilinear')
        ...
        out2 = (out**2).sum(1)
        out2 = F.interpolate(out2[None,:], size =(28,28), mode='bilinear')
        #out2 = F.interpolate(out[None,:], size =(128,14,14), mode='trilinear')
        ...
        out = self.relu(out)
        out3 = (out**2).sum(1)
        out3 = F.interpolate(out3[None,:], size =(28,28), mode='bilinear')
        # out3 = F.interpolate(out[None,:], size =(128,14,14), mode='trilinear')
        all_out = torch.cat((all_out, out1, out2, out3))
        #all_out = all_out + [out1, out2, out3]
        return all_out, out 
...
class ResNet(nn.Module):
    ...
    def _forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        x = self.relu(x)
        #all_output = x[None,:]
        all_output = (x**2).sum(1)
        all_output = F.interpolate(all_output[None,:], size =(28,28), mode='bilinear')
        x = self.maxpool(x)

        all_output, x = self.layer1((all_output, x))
        all_output, x = self.layer2((all_output, x))
        all_output, x = self.layer3((all_output, x))
        all_output, x = self.layer4((all_output, x), fake_relu=fake_relu, no_relu=no_relu)
