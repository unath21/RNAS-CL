...
def compute_model_complexity(
    ...
    if verbose:
        ...
        print('  {}'.format('-' * num_udscore))
        print(
            'Total (params={:,} Million, flops={:,} Billion)'.format(
                total_params/(1000*1000), total_flops/(1000*1000*1000)
            )
        )
        print('  {}'.format('-' * num_udscore))
...
'''
##### An example on computating model complexity
import torch
from ptflops import get_model_complexity_info
from modeling.baseline import Baseline
from utils import model_complexity
model = Baseline(751, 1, '/home/ywan1053/reid-strong-baseline-master/log/market1501/mobilenetv3_large/mobilenetv3_large_model_120.pth', 'bnneck', 'after', 'resnet50', 'self')
model_complexity.compute_model_complexity(model, (1, 3, 256, 128), verbose=True)
'''
